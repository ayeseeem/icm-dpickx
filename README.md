...
or `--number` (or `-n`)
in which case it will try to parse the content as a number or as an integer,
and fail if it cannot.
`-number` is more liberal in what it accepts than `--integer`.
...
- [x] Fix [Issue 1](https://github.com/ayeseeem/icm-dpickx/issues/1#issue-733753076)
